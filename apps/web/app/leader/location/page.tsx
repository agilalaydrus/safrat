...
export default function LeaderLocationPage() {
  ...
  async function submit() {
    ...
    setNotice("");
    try {
      const result = await groupLeaderClient.updateMyGroupCity({ groupId, city, location, notes: "" });
      setCurrentCity(result.currentCity);
      setLastUpdate(result.lastUpdate?.toDate());
      setCity("");
      setLocation("");
      setNotice("Lokasi rombongan berhasil diperbarui — jamaah dan operator otomatis mendapat notifikasi.");
    } catch (caught) {
  ...
  return (
    <main style={page}>
      <p style={eyebrow}>LOKASI ROMBONGAN</p>
      ...
      <section style={form}>
        ...
        <label style={fieldLabel}>Detail (opsional)</label>
        ...
        <button disabled={saving || !city} onClick={() => void submit()} style={primary}>
          {saving ? "Menyimpan..." : "Update Lokasi Rombongan"}
        </button>
        <p style={{ margin: 0, fontSize: 12, color: "var(--color-warm-400)" }}>Satu tap ini otomatis memperbarui status perjalanan seluruh jamaah di rombongan Anda, dan terlihat oleh operator serta keluarga jamaah — tidak perlu langkah tambahan.</p>
      </section>
    </main>
  );
}
...
const primary: React.CSSProperties = { minHeight: 48, border: 0, borderRadius: 10, background: "var(--color-gold-500)", color: "var(--color-warm-900)", fontWeight: 700, marginTop: 4 };
