...
async function seedRefundPayout(): Promise<{ requestId: string; pilgrimId: string; pilgrimName: string }> {
  ...
  const [request] = await query<{ id: string }>(
    `INSERT INTO pilgrim_refund_payout_requests
     ...
     VALUES ($1,$2,350000,'CASH','Serah terima di kantor travel',$3,'e2e-pilgrim')
     RETURNING id::text AS id`,
...
test.describe("money screens render", () => {
  ...
  test("refund payout moves from requested to paid and debits the ledger once", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.getByRole("heading", { name: /Pencairan Saldo Refund/i })).toBeVisible();
      ...
      await card.getByLabel(/Bukti serah terima/i).setInputFiles({
        name: "bukti-serah-terima.png",
        mimeType: "image/png",
        // A self-contained 1x1 PNG keeps the test independent from screenshots
        // generated by another test (which may run in a different worker).
        buffer: Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=", "base64"),
      });
      await expect(page.getByText(/Bukti serah terima berhasil diunggah/i)).toBeVisible();
