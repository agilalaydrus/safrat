...
// NEXT_PUBLIC_APP_URL. A relative /sign-in href keeps the visitor on
// vacana.tawafiqhub.id, where every /api/auth call is then cross-origin and
// blocked by CORS — a sign-in form that cannot sign anyone in. Always send
// them to the apex so the auth client is same-origin with the page.
const SIGN_IN_URL = `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/sign-in`;
...
function formatArticleDate(value?: string | Timestamp) { if (!value) return ""; const date = typeof value === "string" ? new Date(value) : value.toDate(); return Number.isNaN(date.getTime()) ? "" : date.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }); }
