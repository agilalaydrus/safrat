...
export function AuthForm({mode}:{mode:Mode}) { const router=useRouter(); const [error,setError]=useState<string|null>(null); const [isSubmitting,setSubmitting]=useState(false); const [focused,setFocused]=useState(""); const [checkEmail,setCheckEmail]=useState<string|null>(null); const [unverifiedEmail,setUnverifiedEmail]=useState<string|null>(null); const [resending,setResending]=useState(false);
 ...
 const [awaitingCode,setAwaitingCode]=useState(false); const [totpCode,setTotpCode]=useState("");
 ...
 // The plugin does not issue a session until this succeeds — the pending one
 // is discarded — so a session existing at all already means the code was
 // verified. Nothing downstream has to re-check it.
 async function submitCode(event:FormEvent<HTMLFormElement>){event.preventDefault();setError(null);
  const code=totpCode.replace(/\D/g,"");
  if(code.length!==6){setError("Masukkan 6 digit dari aplikasi authenticator.");return;}
  setSubmitting(true);
  try{const result=await authClient.twoFactor.verifyTotp({code});
   if(result.error){setError("Kode tidak cocok. Coba kode terbaru dari aplikasi Anda.");return;}
   await completeSignIn();}
  catch{setError("Gagal memverifikasi kode. Silakan coba lagi.");}
 ...
 if(awaitingCode){return <form onSubmit={submitCode} style={{display:"grid",gap:14}}>
  <div style={{display:"grid",gap:6}}>
   <p style={{fontWeight:700,fontSize:15,margin:0}}>Verifikasi dua langkah</p>
   <p style={{fontSize:13,color:"var(--color-warm-500)",margin:0}}>Masukkan 6 digit dari aplikasi authenticator Anda.</p>
  </div>
  <input inputMode="numeric" autoComplete="one-time-code" autoFocus maxLength={6} value={totpCode}
   onChange={(e)=>{setTotpCode(e.target.value);setError(null);}}
   style={{...inputStyle,borderWidth:1,borderStyle:"solid",borderColor:"var(--color-cream-500)",letterSpacing:"0.35em",fontSize:20,textAlign:"center"}} />
  {error&&<p role="alert" style={{color:"var(--color-danger-600)",fontSize:13,margin:0}}>{error}</p>}
  <button type="submit" disabled={isSubmitting} style={submitStyle}>{isSubmitting?"Memverifikasi...":"Verifikasi"}</button>
 </form>;}
