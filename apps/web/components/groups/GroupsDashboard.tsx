...
import { IconMessageCircle,IconPencil,IconPlus,IconTrash,IconUserCheck,IconUsers,IconUsersGroup } from "@tabler/icons-react";
...
export default function GroupsDashboard(){
  const[seasons,setSeasons]=useState<{id:string;name:string;isActive:boolean}[]>([]),[seasonId,setSeasonId]=useState(""),[groups,setGroups]=useState<Group[]>([]),[open,setOpen]=useState(false),[edit,setEdit]=useState<Group|undefined>(),[notice,setNotice]=useState(""),[chatGroup,setChatGroup]=useState<Group|undefined>(),[rosterGroup,setRosterGroup]=useState<Group|undefined>();
  const load=async(id=seasonId)=>{if(!id)return;try{setGroups((await groupClient.listGroups({seasonId:id})).groups)}catch{setNotice("Gagal memuat data rombongan.")}};
  ...
  return <main style={page}>
    <header style={header}><div><p style={eyebrow}>OPERASIONAL / ROMBONGAN</p><h1 style={title}>Rombongan</h1></div>
      <div style={actions}><select value={seasonId} onChange={e=>setSeasonId(e.target.value)} style={select}>{seasons.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><button style={emerald} onClick={()=>{setEdit(undefined);setOpen(true)}}><IconPlus size={18}/>Tambah Rombongan</button></div>
    </header>
    <div className="gold-divider"/>
    {notice&&<p style={{color:"var(--color-danger-600)"}}>{notice}</p>}
    <div style={stats}>{[["Total Rombongan",groups.length],["Total Jamaah Terkelompok",groups.reduce((n,g)=>n+g.pilgrimCount,0)],["Rombongan Tanpa Ketua",groups.filter(g=>!g.leaderId).length]].map(x=><div style={stat} key={String(x[0])}><small>{x[0]}</small><strong>{x[1]}</strong></div>)}</div>
