...
export default function GroupsDashboard(){
  ...
  return <main style={page}>
    ...
    <div style={stats}>{[["Total Grup",groups.length],["Total Jamaah Terkelompok",groups.reduce((n,g)=>n+g.pilgrimCount,0)],["Grup Tanpa Muttawwif",groups.filter(g=>!g.leaderId).length]].map(x=><div style={stat} key={String(x[0])}><small>{x[0]}</small><strong>{x[1]}</strong></div>)}</div>
    {groups.length?<div style={grid}>{groups.map(g=>
      <article style={{...card,cursor:"pointer"}} key={g.id} onClick={()=>router.push(`/dashboard/groups/${g.id}?seasonId=${seasonId}`)}>
        <div style={row}><h2 style={{margin:0,fontSize:18}}>{g.name}</h2><span style={capBadge}>{g.pilgrimCount}/{g.capacity} jamaah</span></div>
        <div style={barTrack}><div style={{...barFill,width:`${g.capacity?Math.min(100,(g.pilgrimCount/g.capacity)*100):0}%`}}/></div>
        <p style={leaderRow}><IconUserCheck size={15}/>{g.leaderName||"Belum ada Muttawwif"}</p>
        <div style={row} onClick={(e)=>e.stopPropagation()}><span><button style={ghost} onClick={()=>setAddJamaahGroup(g)}><IconUserPlus size={15}/>Tambah Jamaah</button><button style={ghost} onClick={()=>setRosterGroup(g)}><IconUsers size={15}/>Lihat Anggota</button><button style={ghost} onClick={()=>setChatGroup(g)}><IconMessageCircle size={15}/>Lihat Chat</button></span><span><button style={ghost} onClick={()=>{setEdit(g);setOpen(true)}}><IconPencil size={15}/>Ubah</button><RoleGate require={["owner","admin"]}><button style={{...ghost,color:"var(--color-danger-600)"}} onClick={async()=>{if(window.confirm(`Hapus grup ${g.name}? Jamaah akan dilepas dari grup, bukan dihapus.`)){await groupClient.deleteGroup({groupId:g.id});void load()}}}><IconTrash size={15}/>Hapus</button></RoleGate></span></div>
      </article>
    )}</div>:<section style={empty}><IconUsersGroup size={48} color="var(--color-warm-400)"/><h2 style={{margin:0}}>Belum ada grup</h2><p>Buat grup untuk mengelompokkan jamaah dan menetapkan Muttawwif.</p><button style={{...gold,border:"1px solid var(--color-emerald-700)",background:"transparent",color:"var(--color-emerald-900)"}} onClick={()=>setOpen(true)}>Tambah Grup</button></section>}
