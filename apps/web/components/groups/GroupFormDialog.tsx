...
export default function GroupFormDialog({open,seasonId,initial,onClose,onSaved}:P){
  ...
  async function submit(event:FormEvent){
    ...
    }catch(caught){setError(caught instanceof Error?caught.message:"Unable to save group.")}
    finally{setSaving(false)}
  }
  return <div style={o}><aside style={s}><div style={h}><div><p style={ey}>GROUP</p><h2 style={{margin:0}}>{initial?"Edit group":"Add group"}</h2></div><button className="btn-close-sheet" onClick={onClose} style={x}><IconX size={18}/></button></div>
    <div style={b}><form id="group-form" onSubmit={submit} style={{display:"grid",gap:16}}>
      <label style={{display:"grid",gap:6}}><span style={lab}>Group name</span><input className="safrat-input" value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. GROUP-A" style={i}/></label>
      <label style={{display:"grid",gap:6}}><span style={lab}>Capacity</span><input className="safrat-input" type="number" min="1" value={capacity} onChange={e=>setCapacity(e.target.value)} style={i}/></label>
      {initial&&<label style={{display:"grid",gap:6}}><span style={lab}>Group leader</span><select className="safrat-input" value={leaderId} onChange={e=>setLeaderId(e.target.value)} style={i}><option value="">Not assigned</option>{members.map(m=><option key={m.userId} value={m.userId}>{m.name} ({m.email})</option>)}</select></label>}
      {!initial&&<p style={{margin:0,fontSize:12,color:"var(--color-warm-400)"}}>You can assign a group leader after creating the group.</p>}
      {error&&<p style={err}>{error}</p>}
    </form></div>
    <div style={foot}><button form="group-form" disabled={saving} style={primary}>{saving?"Saving...":"Save group"}</button></div>
  </aside></div>
}
