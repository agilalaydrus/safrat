...
import { uploadStorefrontAudio, uploadStorefrontImage } from "@/lib/storefront-upload";
import { DEFAULT_STOREFRONT_THEME } from "@/components/storefront/TenantStorefront";
import type { StorefrontArticle, StorefrontAssurancePillar, StorefrontContent, StorefrontOperatingHour, StorefrontPackage, StorefrontPublicPackage, StorefrontSeason, StorefrontSocialLink, StorefrontTheme } from "@/components/storefront/TenantStorefront";

type Tab = "brand" | "packages" | "gallery" | "social" | "trust" | "publishing";

const EMPTY_CONTENT: StorefrontContent = { brandColor: "#059669", theme: { ...DEFAULT_STOREFRONT_THEME }, packages: [], publicPackages: [], gallery: [], socialLinks: [], testimonials: [], faqs: [], news: [], blogPosts: [], assurancePillars: [], operatingHours: [], backgroundMusicVolume: 35 };
const SOCIAL_PLATFORMS: { value: StorefrontSocialLink["platform"]; label: string }[] = [
  { value: "instagram", label: "Instagram" }, { value: "tiktok", label: "TikTok" },
  { value: "youtube", label: "YouTube" }, { value: "facebook", label: "Facebook" },
  { value: "linkedin", label: "LinkedIn" }, { value: "threads", label: "Threads" },
  { value: "x", label: "X" }, { value: "whatsapp", label: "WhatsApp" },
];
const DEFAULT_ASSURANCE_EDITOR: StorefrontAssurancePillar[] = [
...
export default function OperatorProfilePanel() {
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const editor = await operatorClient.saveMyStorefrontDraft({ content: toRPCContent(content) as never, expectedRevision: draftRevision });
      setDraftRevision(editor.draftRevision);
      setPublishedRevision(editor.publishedRevision);
      setPublishedAt(editor.publishedAt?.toDate().toLocaleString("id-ID") ?? "");
      setStorageUsage({ usedBytes: Number(editor.storageUsedBytes), quotaBytes: Number(editor.storageQuotaBytes), assetCount: editor.storageAssetCount, pendingCount: editor.storagePendingCount });
  ...
  const publish = async () => {
    ...
    try {
      validateContent(content);
      const saved = await operatorClient.saveMyStorefrontDraft({ content: toRPCContent(content) as never, expectedRevision: draftRevision });
  ...
  return <div style={{ display: "grid", gap: 18 }}>
    ...
    <nav style={tabs} aria-label="Bagian CMS">
      ...
      <TabButton active={tab === "social"} onClick={() => setTab("social")}>Sosial &amp; Audio</TabButton>
    ...
    {tab === "social" && <div style={{ display: "grid", gap: 16 }}>
      <Section title="Sosial media resmi" description="Tampilkan maksimal delapan kanal resmi. Satu platform hanya dapat dipakai sekali agar pengunjung tidak bingung.">
        <Field label="Judul section"><input value={content.socialTitle ?? ""} onChange={(event) => patch(setContent, { socialTitle: event.target.value })} maxLength={120} style={input} placeholder={`Terhubung lebih dekat bersama ${content.displayName || operator.name || "kami"}`} /></Field>
        <Field label="Deskripsi"><textarea value={content.socialDescription ?? ""} onChange={(event) => patch(setContent, { socialDescription: event.target.value })} maxLength={600} rows={3} style={textarea} /></Field>
        {(content.socialLinks ?? []).map((item, index) => <SocialLinkEditor key={`${item.platform}-${index}`} value={item} occupied={(content.socialLinks ?? []).map((link) => link.platform)} onChange={(value) => setContent({ ...content, socialLinks: content.socialLinks?.map((entry, itemIndex) => itemIndex === index ? value : entry) })} onRemove={() => setContent({ ...content, socialLinks: content.socialLinks?.filter((_, itemIndex) => itemIndex !== index) })} />)}
        {(content.socialLinks?.length ?? 0) < SOCIAL_PLATFORMS.length && <Add onClick={() => { const occupied = new Set((content.socialLinks ?? []).map((item) => item.platform)); const platform = SOCIAL_PLATFORMS.find((item) => !occupied.has(item.value))?.value; if (platform) setContent({ ...content, socialLinks: [...(content.socialLinks ?? []), { platform, label: SOCIAL_PLATFORMS.find((item) => item.value === platform)?.label || platform, handle: "", url: "" }] }); }}>Tambah Kanal Sosial</Add>}
      </Section>
      <Section title="Musik latar" description="Upload MP3 maksimal 10 MB. Musik diputar berulang, dapat dibisukan, dan tetap mengikuti kebijakan autoplay browser pengunjung.">
        <label style={checkboxLabel}><input type="checkbox" checked={content.backgroundMusicEnabled ?? false} onChange={(event) => patch(setContent, { backgroundMusicEnabled: event.target.checked })} /> Aktifkan musik latar di landing page</label>
        <Field label="Judul musik"><input value={content.backgroundMusicTitle ?? ""} onChange={(event) => patch(setContent, { backgroundMusicTitle: event.target.value })} maxLength={120} style={input} placeholder="Nasyid perjalanan" /></Field>
        <Field label={`Volume awal ${content.backgroundMusicVolume ?? 35}%`}><input type="range" min={5} max={100} step={5} value={content.backgroundMusicVolume ?? 35} onChange={(event) => patch(setContent, { backgroundMusicVolume: Number(event.target.value) })} style={{ width: "100%", accentColor: "var(--color-emerald-900)" }} /><span style={hintStyle}>Gunakan volume rendah agar tidak mengganggu. Pengunjung tetap dapat membisukan musik.</span></Field>
        <AudioField value={content.backgroundMusicUrl ?? ""} onChange={(backgroundMusicUrl) => patch(setContent, { backgroundMusicUrl })} />
        <p style={hintStyle}>Browser dapat menolak autoplay bersuara sampai pengunjung berinteraksi. Saat itu tombol Putar Musik tampil sebagai fallback, lalu preferensi mute disimpan di perangkat pengunjung.</p>
      </Section>
    </div>}

    {tab === "trust" && <div style={{ display: "grid", gap: 16 }}>
      <Section title="Empat pilar jaminan" description="Empat pesan singkat ini tampil tepat setelah hero. Judul harus berbeda agar setiap pilar memiliki fungsi yang jelas.">
        {((content.assurancePillars?.length === 4 ? content.assurancePillars : DEFAULT_ASSURANCE_EDITOR)).map((item, index) => <div key={index} style={twoColumns}><Field label={`Pilar ${index + 1}`}><input value={item.title} onChange={(event) => patch(setContent, { assurancePillars: updateAssurancePillar(content.assurancePillars, index, { title: event.target.value }) })} maxLength={80} style={input} /></Field><Field label="Keterangan"><input value={item.description} onChange={(event) => patch(setContent, { assurancePillars: updateAssurancePillar(content.assurancePillars, index, { description: event.target.value }) })} maxLength={160} style={input} /></Field></div>)}
      </Section>
    ...
    {tab === "publishing" && <div style={{ display: "grid", gap: 16 }}>
      <Section title="Profil perusahaan" description="Cerita yang lebih lengkap untuk halaman Tentang Kami.">
        ...
        <ImageField label="Foto Tentang Kami" value={content.aboutImageUrl ?? ""} kind={StorefrontAssetKind.ABOUT} hint="Gunakan foto dokumenter asli travel. Gambar diproses ke WebP maksimal 2000 px." onChange={(aboutImageUrl) => patch(setContent, { aboutImageUrl })} />
        {content.aboutImageUrl && <div style={twoColumns}><Field label="Alt text foto"><input value={content.aboutImageAlt ?? ""} onChange={(event) => patch(setContent, { aboutImageAlt: event.target.value })} maxLength={160} style={input} placeholder="Pembimbing mendampingi jamaah di Tanah Suci" /><span style={hintStyle}>Wajib untuk aksesibilitas saat foto khusus digunakan.</span></Field><Field label="Caption foto"><input value={content.aboutImageCaption ?? ""} onChange={(event) => patch(setContent, { aboutImageCaption: event.target.value })} maxLength={160} style={input} placeholder="Pendampingan yang hadir di setiap tahap perjalanan" /></Field></div>}
      ...
      <Section title="Jam operasional" description="Atur satu sampai tujuh kelompok jadwal. Label hari yang sama tidak boleh diulang.">
        {(content.operatingHours ?? []).map((item, index) => <div key={`${item.dayLabel}-${index}`} style={listEditor}><div style={twoColumns}><Field label="Hari layanan"><input value={item.dayLabel} onChange={(event) => patch(setContent, { operatingHours: updateOperatingHour(content.operatingHours, index, { dayLabel: event.target.value }) })} maxLength={80} style={input} placeholder="Senin sampai Jumat" /></Field><Field label="Jam dan zona waktu"><input value={item.hoursLabel} onChange={(event) => patch(setContent, { operatingHours: updateOperatingHour(content.operatingHours, index, { hoursLabel: event.target.value }) })} maxLength={80} style={input} placeholder="09.00 sampai 17.00 WIB" /></Field></div>{(content.operatingHours?.length ?? 0) > 1 && <Remove onClick={() => patch(setContent, { operatingHours: content.operatingHours?.filter((_, itemIndex) => itemIndex !== index) })} />}</div>)}
        {(content.operatingHours?.length ?? 0) < 7 && <Add onClick={() => patch(setContent, { operatingHours: [...(content.operatingHours ?? []), { dayLabel: "", hoursLabel: "" }] })}>Tambah Jam Operasional</Add>}
      </Section>
      ...
      <ArticleEditor title="Berita" kind="berita" value={content.news ?? []} onChange={(news) => setContent({ ...content, news })} />
      <ArticleEditor title="Blog SEO" kind="blog" value={content.blogPosts ?? []} onChange={(blogPosts) => setContent({ ...content, blogPosts })} />
    </div>}
...
/**
 * Price entry for a package, in one place because both editors need the same
 * rule explained the same way.
 *
 * The number is what makes a price eligible to appear in Google results, and it
 * also becomes the price the page shows — so a visitor and a search engine can
 * never be told different figures. That matching is not a nicety: a marked-up
 * price that disagrees with the visible one is a policy violation, and the
 * penalty lands on the travel agency's own domain.
 */
function PriceFields<T extends { priceLabel?: string; priceFromIdr?: number | string }>(
  { value, onChange }: { value: T; onChange: (next: T) => void },
) {
  const amount = typeof value.priceFromIdr === "string" ? Number(value.priceFromIdr) : value.priceFromIdr;
  const hasAmount = amount !== undefined && Number.isFinite(amount) && Number(amount) > 0;
  return (
    <>
      <Field label="Harga mulai (angka)">
        <input
          type="number"
          min={0}
          step={100000}
          value={hasAmount ? String(amount) : ""}
          onChange={(event) => onChange({ ...value, priceFromIdr: event.target.value === "" ? 0 : Number(event.target.value) })}
          style={input}
          placeholder="28000000"
        />
        <small style={priceHint}>
          {hasAmount
            ? `Ditampilkan sebagai ${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(Number(amount))}, dan bisa muncul di hasil pencarian Google.`
            : "Isi angkanya agar harga bisa tampil di hasil pencarian Google. Kosongkan kalau harga hanya diberikan lewat percakapan."}
        </small>
      </Field>
      <Field label="Teks harga (dipakai bila angka kosong)">
        <input
          value={value.priceLabel ?? ""}
          onChange={(event) => onChange({ ...value, priceLabel: event.target.value })}
          maxLength={80}
          style={{ ...input, opacity: hasAmount ? 0.5 : 1 }}
          placeholder="Hubungi kami"
          disabled={hasAmount}
        />
        <small style={priceHint}>
          {hasAmount ? "Tidak dipakai selama angka di sebelah terisi." : "Mis. \u201cHubungi kami\u201d."}
        </small>
      </Field>
    </>
  );
...
function AudioField({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  const upload = async (file?: File) => {
    ...
    try {
      const result = await uploadStorefrontAudio(file);
      onChange(result.url);
      setInfo(`MP3 tersimpan, ${formatBytes(result.bytes)}`);
    } catch (cause) { setInfo(message(cause, "Upload musik gagal.")); }
    finally { setUploading(false); }
  };
  return <Field label="File musik"><div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}><label style={{ ...uploadButton, opacity: uploading ? 0.6 : 1 }}><IconMusic size={17} /> {uploading ? "Mengunggah..." : "Pilih MP3"}<input type="file" accept="audio/mpeg,.mp3" disabled={uploading} onChange={(event) => { void upload(event.target.files?.[0]); event.currentTarget.value = ""; }} style={{ display: "none" }} /></label>{value && <button type="button" onClick={() => { onChange(""); setInfo(""); }} style={iconButton} aria-label="Hapus musik latar"><IconTrash size={17} /></button>}</div>{value && <div style={audioPreview}><audio src={value} controls preload="metadata" /><span>{value}</span></div>}<span style={hintStyle}>{info || "Format MP3, maksimal 10 MB. Pilih audio yang Anda miliki hak penggunaannya."}</span></Field>;
}

function SocialLinkEditor({ value, occupied, onChange, onRemove }: { value: StorefrontSocialLink; occupied: StorefrontSocialLink["platform"][]; onChange: (value: StorefrontSocialLink) => void; onRemove: () => void }) {
  return <div style={listEditor}><div style={twoColumns}><Field label="Platform"><select value={value.platform} onChange={(event) => { const platform = event.target.value as StorefrontSocialLink["platform"]; onChange({ ...value, platform, label: value.label || SOCIAL_PLATFORMS.find((item) => item.value === platform)?.label || platform }); }} style={input}>{SOCIAL_PLATFORMS.map((item) => <option key={item.value} value={item.value} disabled={item.value !== value.platform && occupied.includes(item.value)}>{item.label}</option>)}</select></Field><Field label="Label tombol"><input value={value.label} onChange={(event) => onChange({ ...value, label: event.target.value })} maxLength={80} style={input} placeholder="Kunjungi Instagram Kami" /></Field></div><div style={twoColumns}><Field label="Handle akun"><input value={value.handle ?? ""} onChange={(event) => onChange({ ...value, handle: event.target.value })} maxLength={120} style={input} placeholder="@namatravel" /></Field><Field label="URL resmi"><input type="url" value={value.url} onChange={(event) => onChange({ ...value, url: event.target.value })} maxLength={2048} style={input} placeholder="https://instagram.com/namatravel" /></Field></div><Remove onClick={onRemove} /></div>;
}
...
function ArticleEditor({ title, kind, value, onChange }: { title: string; kind: "berita" | "blog"; value: StorefrontArticle[]; onChange: (value: StorefrontArticle[]) => void }) {
  const update = (index: number, patchValue: Partial<StorefrontArticle>) => onChange(value.map((entry, itemIndex) => itemIndex === index ? { ...entry, ...patchValue } : entry));
  return <Section title={title} description="Lengkapi artikel, tinjau hasil Google, lalu publikasikan bersama draft storefront.">
    {value.length === 0 && <Empty text={`Belum ada ${title.toLowerCase()}.`} />}
    {value.map((item, index) => <details key={item.id || index} style={articleEditor}>
      <summary style={articleSummary}><span>{item.title.trim() || `${title} baru`}</span><small>{item.slug ? `/${kind}/${item.slug}` : "Slug belum diisi"}</small></summary>
      <div style={articleEditorBody}>
        <div style={twoColumns}>
          <Field label="Judul artikel"><input value={item.title} onChange={(event) => { const nextTitle = event.target.value; const shouldSyncSlug = !item.slug || item.slug === slugify(item.title); update(index, { title: nextTitle, ...(shouldSyncSlug ? { slug: slugify(nextTitle) } : {}) }); }} maxLength={180} style={input} /></Field>
          <Field label="Slug URL"><input value={item.slug} onChange={(event) => update(index, { slug: slugify(event.target.value) })} minLength={3} maxLength={180} spellCheck={false} style={input} placeholder="panduan-umrah-keluarga" /><span style={hintStyle}>Gunakan minimal 3 karakter. Slug harus unik di Berita dan Blog.</span></Field>
        </div>
        <ImageField label="Foto sampul" value={item.coverImageUrl ?? ""} kind={StorefrontAssetKind.ARTICLE} hint="Foto lanskap 16:9 disarankan. Otomatis dikompresi ke WebP maksimal 2000 px." onChange={(coverImageUrl) => update(index, { coverImageUrl })} />
        {item.coverImageUrl && <Field label="Alt text foto"><input value={item.altText ?? ""} onChange={(event) => update(index, { altText: event.target.value })} maxLength={160} style={input} placeholder="Jamaah bersiap memasuki Masjidil Haram" /><span style={hintStyle}>Wajib saat foto sampul digunakan. Jelaskan isi foto secara singkat.</span></Field>}
        <div style={twoColumns}>
          <Field label="Penulis"><input value={item.author ?? ""} onChange={(event) => update(index, { author: event.target.value })} maxLength={120} style={input} placeholder="Tim redaksi travel" /></Field>
          <Field label="Tanggal publikasi"><input type="date" value={articleDateInput(item.publishedAt)} onChange={(event) => update(index, { publishedAt: timestampFromDateInput(event.target.value) })} style={input} /></Field>
        </div>
        <Field label="Ringkasan"><textarea value={item.excerpt ?? ""} onChange={(event) => update(index, { excerpt: event.target.value })} maxLength={320} rows={3} style={textarea} /><span style={characterCount}>{(item.excerpt ?? "").length}/320 karakter</span></Field>
        <Field label="Isi artikel"><textarea value={item.body} onChange={(event) => update(index, { body: event.target.value })} maxLength={20000} rows={12} style={textarea} placeholder="Tulis artikel dengan paragraf pendek agar nyaman dibaca." /><span style={characterCount}>{item.body.length}/20.000 karakter</span></Field>
        <fieldset style={seoFieldset}>
          <legend style={themeLegend}>Optimasi pencarian</legend>
          <Field label="SEO title"><input value={item.seoTitle ?? ""} onChange={(event) => update(index, { seoTitle: event.target.value })} maxLength={70} style={input} placeholder={item.title || "Judul yang tampil di Google"} /><span style={characterCount}>{(item.seoTitle ?? "").length}/70 karakter</span></Field>
          <Field label="SEO description"><textarea value={item.seoDescription ?? ""} onChange={(event) => update(index, { seoDescription: event.target.value })} maxLength={160} rows={3} style={textarea} placeholder={item.excerpt || "Ringkasan singkat untuk hasil pencarian"} /><span style={characterCount}>{(item.seoDescription ?? "").length}/160 karakter</span></Field>
          <div style={googlePreview} aria-label="Preview hasil Google"><span style={googleLabel}>Preview Google</span><span style={googleURL}>https://travel.tawafiqhub.id/{kind}/{item.slug || "slug-artikel"}</span><strong>{item.seoTitle || item.title || "Judul artikel"}</strong><p>{item.seoDescription || item.excerpt || "Deskripsi artikel akan tampil di bagian ini."}</p></div>
        </fieldset>
        <Remove onClick={() => { if (window.confirm(`Hapus ${title.toLowerCase()} ini dari draft?`)) onChange(value.filter((_, itemIndex) => itemIndex !== index)); }} />
      </div>
    </details>)}
    {value.length < 30 && <Add onClick={() => onChange([...value, { id: crypto.randomUUID(), title: "", slug: "", excerpt: "", body: "", author: "", publishedAt: Timestamp.fromDate(new Date()) }])}>Tambah {title}</Add>}
  </Section>;
}

function toDraftContent(value: unknown): StorefrontContent { if (!value || typeof value !== "object") return { ...EMPTY_CONTENT, theme: { ...DEFAULT_STOREFRONT_THEME } }; const raw = value as StorefrontContent; return { displayName: raw.displayName ?? "", logoUrl: raw.logoUrl ?? "", description: raw.description ?? "", whatsappNumber: raw.whatsappNumber ?? "", managerWhatsapp: raw.managerWhatsapp ?? "", contactEmail: raw.contactEmail ?? "", website: raw.website ?? "", address: raw.address ?? "", city: raw.city ?? "", mapUrl: raw.mapUrl ?? "", foundedYear: raw.foundedYear, tagline: raw.tagline ?? "", trustBadges: [...(raw.trustBadges ?? [])], brandColor: raw.brandColor || "#059669", theme: { ...DEFAULT_STOREFRONT_THEME, ...(raw.theme ?? {}), accentColor: raw.theme?.accentColor || raw.brandColor || DEFAULT_STOREFRONT_THEME.accentColor }, heroEyebrow: raw.heroEyebrow ?? "", heroTitle: raw.heroTitle ?? "", heroSubtitle: raw.heroSubtitle ?? "", heroImageUrl: raw.heroImageUrl ?? "", packages: raw.packages?.map((item) => ({ seasonId: item.seasonId, imageUrl: item.imageUrl, summary: item.summary, priceLabel: item.priceLabel, priceFromIdr: item.priceFromIdr, facilities: [...(item.facilities ?? [])], itinerary: item.itinerary?.map((entry) => ({ title: entry.title, description: entry.description })) })) ?? [], publicPackages: raw.publicPackages?.map((item) => ({ ...item, facilities: [...(item.facilities ?? [])], seasons: item.seasons?.map((season) => ({ ...season })) })) ?? [], gallery: raw.gallery?.map((item) => ({ ...item })) ?? [], socialLinks: raw.socialLinks?.map((item) => ({ ...item })) ?? [], socialTitle: raw.socialTitle ?? "", socialDescription: raw.socialDescription ?? "", backgroundMusicUrl: raw.backgroundMusicUrl ?? "", backgroundMusicTitle: raw.backgroundMusicTitle ?? "", backgroundMusicEnabled: raw.backgroundMusicEnabled ?? false, backgroundMusicVolume: raw.backgroundMusicVolume || 35, assurancePillars: (raw.assurancePillars?.length ? raw.assurancePillars : DEFAULT_ASSURANCE_EDITOR).map((item) => ({ ...item })), operatingHours: (raw.operatingHours?.length ? raw.operatingHours : [{ dayLabel: "Senin sampai Sabtu", hoursLabel: "09.00 sampai 17.00 WIB" }]).map((item) => ({ ...item })), testimonials: raw.testimonials?.map((item) => ({ ...item })) ?? [], faqs: raw.faqs?.map((item) => ({ ...item })) ?? [], news: raw.news?.map(normalizeArticle) ?? [], blogPosts: raw.blogPosts?.map(normalizeArticle) ?? [], aboutTitle: raw.aboutTitle ?? "", aboutBody: raw.aboutBody ?? "", aboutImageUrl: raw.aboutImageUrl ?? "", aboutImageAlt: raw.aboutImageAlt ?? "", aboutImageCaption: raw.aboutImageCaption ?? "", seoTitle: raw.seoTitle ?? "", seoDescription: raw.seoDescription ?? "", ogImageUrl: raw.ogImageUrl ?? "", agentTitle: raw.agentTitle ?? "", agentDescription: raw.agentDescription ?? "", agentApplicationsEnabled: raw.agentApplicationsEnabled ?? false }; }
function normalizeArticle(item: StorefrontArticle): StorefrontArticle { return { ...item, publishedAt: normalizeTimestamp(item.publishedAt) }; }
function toRPCContent(content: StorefrontContent): StorefrontContent { return { ...content, news: content.news?.map(normalizeArticle), blogPosts: content.blogPosts?.map(normalizeArticle) }; }
function normalizeTimestamp(value: StorefrontArticle["publishedAt"]): Timestamp | undefined { if (!value) return undefined; if (typeof value !== "string") return value; const date = new Date(value); return Number.isNaN(date.getTime()) ? undefined : Timestamp.fromDate(date); }
function articleDateInput(value: StorefrontArticle["publishedAt"]): string { const timestamp = normalizeTimestamp(value); return timestamp ? timestamp.toDate().toISOString().slice(0, 10) : ""; }
function timestampFromDateInput(value: string): Timestamp | undefined { return value ? Timestamp.fromDate(new Date(`${value}T12:00:00.000Z`)) : undefined; }
function slugify(value: string): string { return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 180); }
...
function updateAssurancePillar(items: StorefrontAssurancePillar[] | undefined, index: number, value: Partial<StorefrontAssurancePillar>) { const current = items?.length === 4 ? items : DEFAULT_ASSURANCE_EDITOR; return current.map((item, itemIndex) => itemIndex === index ? { ...item, ...value } : { ...item }); }
function updateOperatingHour(items: StorefrontOperatingHour[] | undefined, index: number, value: Partial<StorefrontOperatingHour>) { return (items ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, ...value } : item); }
function lines(value: string) { return value.split("\n").map((item) => item.trim()).filter(Boolean); }
function validateContent(content: StorefrontContent) { if (!content.displayName?.trim()) throw new Error("Nama brand wajib diisi."); if (!/^#[0-9a-f]{6}$/i.test(content.brandColor ?? "")) throw new Error("Warna aksen utama harus berupa hex 6 digit."); const theme = { ...DEFAULT_STOREFRONT_THEME, ...(content.theme ?? {}) }; for (const value of Object.values(theme)) { if (!/^#[0-9a-f]{6}$/i.test(value)) throw new Error("Semua warna tema harus berupa hex 6 digit, misalnya #142019."); } const contrastPairs: [string, string, string][] = [[theme.lightHeadingColor, theme.lightBackgroundColor, "judul mode terang terhadap latar"], [theme.lightBodyColor, theme.lightBackgroundColor, "isi mode terang terhadap latar"], [theme.lightMutedColor, theme.lightBackgroundColor, "teks sekunder mode terang terhadap latar"], [theme.lightHeadingColor, theme.lightSurfaceColor, "judul mode terang terhadap kartu"], [theme.lightBodyColor, theme.lightSurfaceColor, "isi mode terang terhadap kartu"], [theme.lightMutedColor, theme.lightSurfaceColor, "teks sekunder mode terang terhadap kartu"], [theme.darkHeadingColor, theme.darkBackgroundColor, "judul mode gelap terhadap latar"], [theme.darkBodyColor, theme.darkBackgroundColor, "isi mode gelap terhadap latar"], [theme.darkMutedColor, theme.darkBackgroundColor, "teks sekunder mode gelap terhadap latar"], [theme.darkHeadingColor, theme.darkSurfaceColor, "judul mode gelap terhadap kartu"], [theme.darkBodyColor, theme.darkSurfaceColor, "isi mode gelap terhadap kartu"], [theme.darkMutedColor, theme.darkSurfaceColor, "teks sekunder mode gelap terhadap kartu"], [theme.heroHeadingColor, theme.darkBackgroundColor, "judul hero"], [theme.heroBodyColor, theme.darkBackgroundColor, "deskripsi hero"]]; const lowContrast = contrastPairs.find(([foreground, background]) => contrastRatio(foreground, background) < 4.5); if (lowContrast) throw new Error(`Kontras ${lowContrast[2]} terlalu rendah. Pilih warna dengan rasio minimal 4.5:1.`); const invalidGallery = content.gallery?.find((item) => !item.altText.trim()); if (invalidGallery) throw new Error("Alt text setiap foto galeri wajib diisi."); const socialLinks = content.socialLinks ?? []; if (socialLinks.length > 8) throw new Error("Sosial media maksimal delapan kanal."); const platforms = new Set<string>(); const urls = new Set<string>(); for (const item of socialLinks) { if (!item.label.trim() || !validWebURL(item.url)) throw new Error("Setiap sosial media wajib memiliki label dan URL HTTP/HTTPS yang valid."); if (platforms.has(item.platform)) throw new Error(`Platform ${item.platform} hanya boleh ditambahkan sekali.`); if (urls.has(item.url.trim())) throw new Error("URL sosial media tidak boleh duplikat."); platforms.add(item.platform); urls.add(item.url.trim()); } if (content.backgroundMusicUrl && !validWebURL(content.backgroundMusicUrl)) throw new Error("URL musik latar tidak valid."); if (content.backgroundMusicEnabled && !content.backgroundMusicUrl) throw new Error("Upload file MP3 sebelum mengaktifkan musik latar."); validateStorefrontDetails(content); validateArticles(content); }
function validateStorefrontDetails(content: StorefrontContent) { if (content.aboutImageUrl && !validWebURL(content.aboutImageUrl)) throw new Error("URL foto Tentang Kami tidak valid."); if (content.aboutImageUrl && !content.aboutImageAlt?.trim()) throw new Error("Alt text foto Tentang Kami wajib diisi."); const volume = content.backgroundMusicVolume ?? 35; if (volume < 5 || volume > 100) throw new Error("Volume awal musik harus antara 5% dan 100%."); const pillars = content.assurancePillars ?? []; if (pillars.length !== 4 || pillars.some((item) => !item.title.trim() || !item.description.trim())) throw new Error("Empat pilar jaminan wajib memiliki judul dan keterangan."); if (new Set(pillars.map((item) => item.title.trim().toLowerCase())).size !== 4) throw new Error("Judul empat pilar jaminan tidak boleh duplikat."); const hours = content.operatingHours ?? []; if (hours.length < 1 || hours.length > 7 || hours.some((item) => !item.dayLabel.trim() || !item.hoursLabel.trim())) throw new Error("Jam operasional wajib berisi satu sampai tujuh jadwal lengkap."); if (new Set(hours.map((item) => item.dayLabel.trim().toLowerCase())).size !== hours.length) throw new Error("Label hari pada jam operasional tidak boleh duplikat."); }
function validateArticles(content: StorefrontContent) { const slugs = new Set<string>(); for (const [section, articles] of [["Berita", content.news ?? []], ["Blog", content.blogPosts ?? []]] as const) { for (const article of articles) { if (!article.id.trim() || !article.title.trim() || !article.body.trim()) throw new Error(`${section}: judul dan isi setiap artikel wajib diisi.`); if (!/^[a-z0-9](?:[a-z0-9-]{1,178}[a-z0-9])?$/.test(article.slug)) throw new Error(`${section}: slug harus terdiri dari 3 sampai 180 karakter kecil, angka, atau tanda minus.`); if (article.coverImageUrl && !validWebURL(article.coverImageUrl)) throw new Error(`${section}: URL foto sampul tidak valid.`); if (article.coverImageUrl && !article.altText?.trim()) throw new Error(`${section}: alt text foto sampul wajib diisi.`); if (slugs.has(article.slug)) throw new Error(`Slug artikel "${article.slug}" sudah dipakai. Slug Berita dan Blog harus unik.`); slugs.add(article.slug); } } }
function validColor(value: string | undefined, fallback: string) { return value && /^#[0-9a-f]{6}$/i.test(value) ? value : fallback; }
function validWebURL(value: string) { try { const url = new URL(value); return url.protocol === "https:" || url.protocol === "http:"; } catch { return false; } }
...
const audioPreview: React.CSSProperties = { display: "grid", gap: 8, minWidth: 0, overflow: "hidden", border: "1px solid var(--color-cream-400)", borderRadius: 8, padding: 10, background: "white", color: "var(--color-warm-400)", fontSize: 10, wordBreak: "break-all" };
const checkboxLabel: React.CSSProperties = { display: "flex", alignItems: "center", gap: 9, color: "var(--color-warm-700)", fontSize: 13, fontWeight: 700 };
...
const articleEditor: React.CSSProperties = { overflow: "hidden", border: "1px solid var(--color-cream-500)", borderRadius: 10, background: "var(--color-cream-100)" };
const articleSummary: React.CSSProperties = { display: "grid", gap: 3, padding: "14px 16px", color: "var(--color-warm-700)", fontWeight: 800, cursor: "pointer" };
const articleEditorBody: React.CSSProperties = { display: "grid", gap: 14, borderTop: "1px solid var(--color-cream-400)", padding: 16 };
const seoFieldset: React.CSSProperties = { display: "grid", gap: 12, minWidth: 0, border: "1px solid var(--color-cream-500)", borderRadius: 10, padding: 14 };
const characterCount: React.CSSProperties = { justifySelf: "end", color: "var(--color-warm-400)", fontSize: 11, fontWeight: 400 };
const googlePreview: React.CSSProperties = { display: "grid", gap: 4, overflow: "hidden", border: "1px solid var(--color-cream-400)", borderRadius: 9, padding: 14, background: "white", color: "var(--color-warm-700)" };
const googleLabel: React.CSSProperties = { color: "var(--color-warm-400)", fontSize: 11, fontWeight: 700 };
const googleURL: React.CSSProperties = { overflow: "hidden", color: "var(--color-emerald-900)", fontSize: 12, fontWeight: 400, textOverflow: "ellipsis", whiteSpace: "nowrap" };
...
const priceHint: React.CSSProperties = { display: "block", marginTop: 4, color: "var(--color-warm-400)", fontSize: 11, lineHeight: 1.5 };
