...
export default function VehicleManifestPanel({ vehicleId, seasonId, movementStatus, mode = "BUS", open, onClose, onChanged }: Props) {
  ...
  async function assignGroup(members: typeof candidates) {
    ...
    try {
      ...
      await refresh();
      onChanged();
      if (toAssign.length < members.length) setNotice(`Kendaraan sudah penuh, baru ${toAssign.length} dari ${members.length} jamaah di grup ini yang tertempatkan.`);
    } catch (caught) {
      setNotice(caught instanceof Error ? caught.message : "Gagal menempatkan grup ini.");
    } finally {
      setWorkingId("");
    }
  }
  ...
  return <div role="dialog" aria-modal="true" aria-label="Manifest kendaraan" style={overlay}><aside style={sheet}>
    ...
    {!full && canAssign && !!groupsWithCandidates.length && <section style={section}><h3 style={sectionTitle}>Tempatkan satu grup</h3>{groupsWithCandidates.map(([groupId, members]) => <div key={groupId} style={person}><div><strong><IconUsersGroup size={16} style={{ verticalAlign: "-3px", marginRight: 4 }} />{groupNames[groupId] ?? "Grup"}</strong><span style={meta}>{members.length} jamaah belum ditempatkan</span></div><button disabled={workingId === "group"} onClick={() => void assignGroup(members)} style={emerald}><IconUserPlus size={17} />Tempatkan grup</button></div>)}</section>}
...
const overlay: React.CSSProperties = { position: "fixed", inset: 0, zIndex: 30, display: "flex", justifyContent: "flex-end", background: "rgba(15,23,42,.52)" };
const sheet: React.CSSProperties = { width: "min(600px,100%)", minHeight: "100%", overflowY: "auto", background: "var(--color-cream-100)", padding: 24, boxShadow: "0 8px 24px rgba(15,23,42,.12)", animation: "sheet-in .2s ease-out" };
