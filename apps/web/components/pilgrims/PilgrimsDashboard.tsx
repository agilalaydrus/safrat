...
export default function PilgrimsDashboard() {
  ...
  const loadAnalytics = () => { if (!seasonId) return; seasonClient.getSeasonAnalytics({ seasonId }).then(setAnalytics).catch(() => {}); };
  const loadExpiringPassports = () => { if (!seasonId) return; pilgrimClient.listPilgrimsWithExpiringPassports({ seasonId, daysThreshold: PASSPORT_WARNING_DAYS }).then((response) => setExpiringPassports(response.pilgrims.length)).catch(() => setExpiringPassports(0)); };
  ...
  const actions_ = useMemo<ActionCenterItem[]>(() => {
    ...
    // an afternoon.
    if (expiringPassports > 0) items.push({
      id: "passports",
      title: `${expiringPassports} paspor kedaluwarsa dalam 6 bulan`,
      ...
      financialImpact: `${expiringPassports} jamaah`,
