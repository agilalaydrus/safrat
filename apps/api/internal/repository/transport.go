...
type Movement struct {
	ID, OperatorID, SeasonID, Name, Origin, Destination, Status string
...
func (r *TransportRepository) CreateMovement(ctx context.Context, op, season, name, origin, destination string, scheduled time.Time) (*Movement, error) {
	o, e := pgUUID(op)
	if e != nil {
		return nil, e
	}
	...
	v, e := r.q.CreateMovement(ctx, db.CreateMovementParams{OperatorID: o, SeasonID: s, Name: name, Origin: origin, Destination: destination, ScheduledAt: pgTimestamp(scheduled)})
	if e != nil {
...
func movement(v db.Movement) *Movement {
	return &Movement{ID: uuidString(v.ID), SeasonID: uuidString(v.SeasonID), OperatorID: uuidString(v.OperatorID), Name: v.Name, Origin: v.Origin, Destination: v.Destination, ScheduledAt: v.ScheduledAt.Time, Status: v.Status, CreatedAt: v.CreatedAt.Time}
}
