...
**Module 5/6 (Group Leader + Pilgrim apps) are PWAs inside `apps/web`, not the native Expo scaffolds.** `apps/mobile-leader`/`apps/mobile-pilgrim` are still empty — there's no simulator/emulator here to verify a real native build, so both were built as mobile-web routes instead: `/pilgrim/[code]` (fully public, `app_access_code` auth, bottom-tab shell in `app/pilgrim/[code]/layout.tsx`) and `/leader` → `/leader/[groupId]` (authenticated, self-scoped to `groups.leader_id = current user`). `GroupLeaderService` in the backend never trusts a `group_id` from the request without calling `GroupLeaderRepository.EnsureLeaderOwnsGroup` first.
